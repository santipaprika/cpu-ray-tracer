...
float GlobalShader::loopScatter(const Ray &r, const std::vector<Shape*> &objList, double smooth, double radius, int currObj) const
{
	int num_it = 20;
	Intersection its;
	double min_t = r.minT;
...
float GlobalShader::recursiveScatter(const Ray &r, const std::vector<Shape*> &objList,  double smooth, double radius, int currObj, int depth) const
{
	...
	if ( currObj > -1)
	{
		...
		if (its.shape->getMaterial().hasSpecular())		//For mirrors
		{
			...
			if (r.depth == n_bounces)
				l_o_ind += computeColor(refl_ray, objList, lsList);
			else
				l_o_ind += Utils::multiplyPerCanal(this->a_t , Vector3D(1.0, 1.0, 1.0)); 
		}
		...
		else if (its.shape->getMaterial().hasDiffuseOrGlossy())
		{
				...
				for (int s = 0; s < lsList.size(); s++) {
					...
					if (!Utils::hasIntersection(r_light, objList))
					{
						...
						else if (its.shape->getMaterial().hasDiffuseOrGlossy())
						{
							...
							l_o += l_o_ind + Utils::multiplyPerCanal(l_i, r);
						}
					}
				}
			}
			else if (r.depth == n_bounces)		// LAST BOUNCE
			{
				for (int s = 0; s < lsList.size(); s++) {
					...
					Vector3D direction = Vector3D(lsList.at(s).getPosition() - its.itsPoint).normalized();
					double max_t = Vector3D(lsList.at(s).getPosition() - its.itsPoint).length();
					Ray r_light = Ray(origen, direction, 0, 0.01, max_t);

					if (!Utils::hasIntersection(r_light, objList))
					{
						Vector3D wi = (lsList.at(s).getPosition() - p).normalized();
						Vector3D l_i = lsList.at(s).getIntensity(p);
						Vector3D r_dir = its.shape->getMaterial().getReflectance(n, wo, wi);
						Vector3D r_ind = Utils::multiplyPerCanal(a_t, its.shape->getMaterial().getDiffuseCoefficient());
						l_o += Utils::multiplyPerCanal(l_i, r_dir + r_ind);
					}
				}
			}
			else	//Other bounces
			{
				...
				Ray r_wr = Ray(origen, wr, r.depth + 1, 0.01);
				Ray r_n = Ray(origen, its.normal, r.depth + 1, 0.01);

				if (Utils::hasIntersection(r_wr, objList))	// NEXT BOUNCE
				{
					Vector3D l_i_ind_wr = computeColor(r_wr, objList, lsList);
					Vector3D l_i_ind_n = computeColor(r_n, objList, lsList);
					Vector3D r_wr = its.shape->getMaterial().getReflectance(n, wo, wr);
					Vector3D r_n = its.shape->getMaterial().getReflectance(n, wo, n);
					l_o_ind += Utils::multiplyPerCanal(l_i_ind, r_wr + r_n);
				}

				l_o_ind = l_o_ind * 1 / (4 * 3.1417);
