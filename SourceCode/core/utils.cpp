...
bool Utils::hasIntersection(const Ray &ray, const std::vector<Shape*> &objectsList)
{
    ...
    for(size_t objIndex = 0; objIndex < objectsList.size(); objIndex ++)
    {
		  ...
		  if (obj->getMaterial().hasScatter())
		  {
			  
		  }
		  else if(obj->rayIntersectP(ray))
			  return true;
...
int Utils::getClosestObject(const Ray &cameraRay, const std::vector<Shape*> &objectsList, Intersection &its)
{
